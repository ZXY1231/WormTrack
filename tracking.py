...
worm_t = [(0, y_mid-l, x_mid-l, img_wormbody)]
worms_t[0] = worm_t
...
for img_name in files:
    ...
    img_new_roi = img_new[y_ori : y_ori+img_wormbody.shape[1],
        x_ori : x_ori+img_wormbody.shape[0]]
    ...
    pos_wormbody = np.where( img_new_roi_th > 50000) 
    pos_wormbody = list(zip(pos_wormbody[0] + y_ori, pos_wormbody[1] + x_ori))
    ...
    if (maxx-minx > 0) and (maxy-miny > 0):
        ...
        l = int(window/2)
        img_wormbody = img_new[y_mid-l:y_mid+l+1, x_mid-l:x_mid+l+1]*body
        ...
        cv.putText(img_new, "Tracking fail", (100,80,), cv.FONT_HERSHEY_SIMPLEX,
            0.75,(0,0,255),2)
    worm_t.append((counter, y_mid-l, x_mid-l, img_wormbody))
    worms_t[worm] = worm_t
    rows = [0, counter, y_mid-l, x_mid-l, img_name]
    spamwriter.writerow(rows)
